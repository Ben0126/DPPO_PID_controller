"""
Phase D: ReinFlow v5.0 — Advantage-Weighted Flow Matching RL Fine-Tuning
Includes:
  - FlowMatchingPolicyV5 (task-conditioned, cross-attention)
  - ValueNetworkV5 (conditioned on 770D task-conditioned global features)
  - Dynamic task flag detection [is_hover, is_recovery] during rollout
  - Positive-Advantage Masking (prevents crash behavior imitation)
"""
...
class ValueNetworkV5(nn.Module):
    """MLP value function conditioned on 770-D global_cond (attended + imu + task)."""

    def __init__(self, global_cond_dim: int = 770, hidden_dim: int = 512):
        super().__init__()
...
def load_demo_subset_v5(h5_path: str, n_episodes: int,
                        T_obs: int, T_pred: int):
    """Load n_episodes from expert demo h5 into RAM for BC loss, returning task labels."""
    ...
    task_cpu = torch.FloatTensor(np.stack(task_buf))
    n = len(img_cpu)
    print(f"  Demo subset: {n_episodes} eps → {n:,} samples | "
          f"img={img_cpu.nbytes/1e6:.0f}MB")
    return img_cpu, imu_cpu, act_cpu, task_cpu
...
def collect_rollout(env: QuadrotorVisualEnv,
    ...
    rollout = {
        ...
        'noise_seqs':   [],   # (N,) × (action_dim, T_pred) float32
        'mu_old':       [],   # (N,) × (action_dim, T_pred) float32
        'imu_data':     [],   # (N,) × (6,) float32
        'rewards':      [],
        'dones':        [],
        'values':       [],
        'task_conds':   [],   # (N,) × (2,) float32
    }
    ...
    while steps_collected < n_steps:
        ...
        # Dynamic Task Detection (Phase B)
        ...
        task_cond_np = np.array([is_hover, is_recovery], dtype=np.float32)
        task_cond_tensor = torch.from_numpy(task_cond_np).float().unsqueeze(0).to(device)

        with torch.no_grad():
            global_cond = policy._encode(img_tensor, imu_tensor, task_cond=task_cond_tensor)  # (1, 770)
        ...
        for a_idx in range(min(T_action, actions_to_exec.shape[0])):
            ...
            rollout['task_conds'].append(task_cond_np.copy())
...
def train(args):
    ...
    if args.quick:
        rl_cfg['num_epochs'] = 1
        rl_cfg['total_updates'] = 2
        rl_cfg['n_rollout_steps'] = 256
        rl_cfg['value_warmup_updates'] = 1
        rl_cfg['demo_episodes'] = 2
        print("[Quick Mode] Running a fast smoke test (2 updates, 256 rollout steps)")
    ...
    # Value network (Attended Vis 256 + IMU 512 + Task Cond 2 = 770)
    global_cond_dim = vis_cfg['feature_dim'] + imu_cfg['feature_dim'] + 2
    value_net = ValueNetworkV5(
    ...
    run_tag   = f"reinflow_v5_{timestamp}"
    ...
    lambda_disp          = rl_cfg.get('lambda_disp', 0.05)
    ...
    positive_mask        = rl_cfg.get('positive_advantage_mask', True)
    ...
    if crash_penalty_rl is not None:
        ...
        print(f"Crash penalty: RL rollout={crash_penalty_rl}")

    # Load expert demos for BC regularization
    demo_img_cpu = demo_imu_cpu = demo_act_cpu = demo_task_cpu = None
    if lambda_bc > 0:
        ...
        demo_img_cpu, demo_imu_cpu, demo_act_cpu, demo_task_cpu = load_demo_subset_v5(
    ...
    if curriculum_enabled:
        ...
        base_env.disturbance_enabled = False
    ...
    print(f"\n{'='*60}")
    print(f"ReinFlow v5.0 — Task Conditioning & Positive-Advantage Masking")
    ...
    print(f"lambda_bc={lambda_bc} | lambda_disp={lambda_disp} | positive_mask={positive_mask}")
    ...
    for update in range(total_updates):
        ...
        task_cpu  = torch.FloatTensor(np.array(rollout['task_conds']))
        ...
        for _ in range(n_epochs):
            ...
            for start in range(0, N, mini_batch):
                ...
                task_gpu = task_cpu[mb].to(device)
                with torch.no_grad():
                    gc = policy._encode(imgs_gpu, imu_gpu, task_cond=task_gpu)
                vp = value_net(gc)
        ...
        if not in_warmup:
            ...
            for _ in range(n_epochs):
                ...
                for start in range(0, N, mini_batch):
                    ...
                    # BC regularization + Dispersive Loss (Phase C)
                    if lambda_bc > 0 and N_demo > 0:
                        ...
                        dtk = demo_task_cpu[dm].to(device)
                        bc_loss = policy.compute_loss(di, du, da, task_cond=dtk, lambda_dispersive=lambda_disp)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ReinFlow v5.0 RL Fine-tuning")
    parser.add_argument('--flow-config',          type=str, default='configs/flow_policy_v5.yaml')
    parser.add_argument('--rl-config',            type=str, default='configs/reinflow_v5.yaml')
    parser.add_argument('--quadrotor-config',     type=str, default='configs/quadrotor_v4.yaml',
                        help='Env config for evaluation')
    parser.add_argument('--rl-quadrotor-config',  type=str, default=None,
                        help='Env config for RL rollout. Defaults to --quadrotor-config if not set.')
    parser.add_argument('--pretrained',           type=str, default=None,
                        help='Path to pretrained FlowMatchingPolicyV5 checkpoint')
