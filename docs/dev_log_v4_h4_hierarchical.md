...
## Chapter 15：v5.0 Joint E2E Training 與 Phase A–D RL 微調（2026-06-03 ~ 2026-06-04）

**接續自** §14.3 的後續方向建議。三個方向中「E2E 訓練」與「RL 新嘗試（positive-advantage mask）」均在本章實施並得出最終結論。

---

### 15.1 v5.0 Joint E2E Training（2026-06-03）

**背景**：v5 Stages A→D（§13）在分離預訓練框架下系統性失敗，根因為 encoder-action alignment。本次改用完全不同策略：解凍視覺編碼器，讓 FlowMatchingPolicyV5 在同一個控制任務上端到端聯合優化。

**實驗配置：**
- 初始權重：H4 BC warm-start（94 個 tensor 成功遷移）
- 視覺編碼器：完全解凍（unfrozen）
- 訓練資料：500 hover episodes + 500 recovery episodes（混合 50:50）
- 超參數：80 epochs, batch_size=256, lr=1e-4（cosine warmup 5 epochs）
- 資料量：446,700 步，~11.5 GB RAM

**收斂曲線（TensorBoard 摘要）：**

| 指標 | Epoch 1 | Best Val (Epoch 30) | Epoch 80 |
|------|---------|---------------------|----------|
| train_flow_loss | 0.1070 | 0.0639 | 0.0564 |
| val_flow_loss | 0.0750 | **0.0642** | 0.0678 |
| train_state_loss | 0.4504 | 0.0656 | 0.0379 |
| val_state_loss | 0.1812 | 0.0815 | 0.0907 |

**Checkpoint：** `checkpoints/flow_policy_v5/20260603_171316/best_model.pt`

**閉環評估結果（30 episodes，old metric）：**

| 模型 | Score | Survive | IAE_steady | Term err | Steps |
|------|-------|---------|------------|----------|-------|
| H4_BC_baseline | 0.171 | 38.8% | 1.325m | 2.426m | 194 |
| Joint_E2E_v5 | 0.073 | **60.1%** | 2.852m | 4.961m | **300.5** |

**結論：**
- 存活率 38.8%→60.1%（+55%），確認視覺編碼器聯合訓練解決了 encoder-action alignment 問題。
- 複合分數下降（0.171→0.073），原因是評估指標的數學不連續性（斷崖效應）加上穩態漂移增大（recovery 資料佔 50% 導致 encoder 對 sub-meter 懸停精度鈍化）。
- 建立**存活率 SOTA**，但複合分數 SOTA 仍為 H4 BC。

---

### 15.2 Phase A：評估指標平滑化（2026-06-04）

**問題：** 舊指標 `max(0, 1 - e/2.0)` 的線性截斷在 SR=0.5 邊界造成 10× 不連續（活得 1% 更久、分數暴跌 10×），且 old formula 沒有物理容差尺度因子。

**修正（已實作至 `scripts/evaluate_hierarchical.py`）：**
```python
stability_score = exp(-IAE_steady / sigma)   # sigma = 2.0m
accuracy_score  = exp(-terminal_err / sigma)
score = SR × (0.6 × stability_score + 0.4 × accuracy_score)
```
移除 Tier 1 gate（不再強制 SR<0.5 走不同公式），整個分數函數在全生存範圍內平滑、連續。

**注意：σ=2.0 下分數不可與舊指標分數直接比較。** 例如 Joint_E2E_v5 在舊指標得 0.073，σ=2.0 得 0.110；H4 BC 從 0.171 降至 0.166。

---

### 15.3 Phase B&C：任務調節 + 端到端色散損失 BC（2026-06-04）

**實作內容：**
- 在 `flow_policy_v5.py` 加入 `task_dim=2` one-hot global condition（`[is_hover, is_recovery]`），由環境狀態（pos_err>1m 或 tilt>15°）動態切換。
- 引入端到端色散損失（Dispersive Loss），在 CNN 特徵層施加斥力防止特徵崩潰。
- 記憶體限制：950+475 episodes 超過 Windows 32GB RAM 上的連續陣列分配上限（>10.2 GiB），下調為 **200 hover + 100 recovery**（~3.1 GiB）。

**訓練結果：**
- 80 epochs，最佳 val_flow_loss **0.066282**
- state aux loss 收斂至 0.03176
- **Checkpoint：** `checkpoints/flow_policy_v5/20260604_141454/best_model.pt`

**評估結果（30 episodes，σ=2.0）：**
- v5_BC：score **0.126**，survive 54.9%，IAE 2.505m，term 4.454m，步數 274

**分析：** 相比 Joint E2E（survive 55.3%，score 0.110 in σ=2.0），Phase B&C 的 BC 複合分數略高（0.126 vs 0.110），但存活率相當。說明任務調節與色散損失對 BC 精度有邊際改善，但並未帶來質變。

---

### 15.4 Phase D：ReinFlow v5 RL 微調（2026-06-04）

**Hypothesis：** positive_advantage_mask 能打破 AWR mode-collapse，讓 RL 有效提升 v5_BC 的懸停精度。

**實作配置（`scripts/train_reinflow_v5.py` + `configs/reinflow_v5.yaml`）：**
- ValueNetworkV5：3 層 MLP，輸入 770D task-conditioned global feature
- GAE：γ=0.99，λ=0.95
- positive_advantage_mask：`mask = (advantages > 0).float()`，僅更新正優勢樣本
- 課程學習：pos_start=0.1m → pos_end（逐步推展）
- Run 1（`reinflow_v5_20260604_134707`）：僅 2 updates，VLoss 929（過高），停止。
- Run 2（`reinflow_v5_20260604_193923`）：700 updates 完整執行。

**Run 2 訓練曲線（TensorBoard 事件檔驗證）：**

| 指標 | first | peak/best | last |
|------|-------|-----------|------|
| mean_reward | -0.3744 | **-0.2161** | -3.0777 |
| frac_pos_adv | 0.54 | 0.70 | 0.45 |
| value_loss | 348.3 | — | 56.4（收斂） |

**閉環評估結果（30 episodes，σ=2.0）：**

| 模型 | Score | Survive | IAE_steady | Term err | RMSE | Steps |
|------|-------|---------|------------|----------|------|-------|
| v5_BC | 0.126 | 54.9% | 2.505m | 4.454m | 2.010m | 274 |
| **v5_RL_best** | **0.130** | 27.9% | **1.224m** | **1.902m** | **0.947m** | 139 |
| v5_RL_final | 0.032 | 4.8% | 0.656m | 1.045m | 0.507m | 24 |

**⚠️ 關鍵警告——v5_RL_best 的低 IAE 是短存活偽象：**
- tier1 通過率僅 **1/30**（平均 139 步，大部分 episode 在 tier1 門檻前即已墜毀）
- 這與本日誌 §1（RMSE 偏誤發現）的機制完全相同，只是換了指標名稱：「飛行時間短 → 沒時間漂移 → IAE 人工偏低」
- 不應將 v5_RL_best 解讀為「精度提升 51%」

**positive_advantage_mask 的貢獻與局限：**

| 面向 | 結果 |
|------|------|
| 即時崩潰防止 | ✓ VLoss 348→56，700 updates 完成（v4.0 Phase 3b VLoss spike 30+ → 即時崩潰） |
| 課程擴展後退化 | ✗ pos_end=0.57m → reward -0.22→-3.08，final survive 4.8% |
| 根本問題 | 課程誘發崩潰與 AWR mode-collapse 本質相同，只是延後爆發 |

**Robustness-Precision Capacity Conflict 確認：**  
在給定參數容量下，「高精度 sub-meter 懸停（需精細小幅動作）」與「廣域 0.57m OOD 危機恢復（需大幅推力輸出）」存在根本性衝突，課程學習只是把矛盾推遲顯現，無法同時優化兩者。

---

### 15.5 最終研究狀態（2026-06-04）

| SOTA | 模型 | 指標 |
|------|------|------|
| 複合分數（舊指標） | **H4 BC** | score 0.171，194 步 |
| 複合分數（σ=2.0） | **H4 BC** | score ~0.166（gemini.md 評估） |
| 存活率 | **Joint_E2E_v5** | 60.1%，300.5 步（20260603 評估） |

v5.0 RL Phase D 已結論。v5 階段全部實驗完成，結果記錄於 [docs/experiment_report_joint_e2e.md](experiment_report_joint_e2e.md)。

**下一步候選方向（尚未執行）：**
1. Phase 4 實機部署（Jetson Orin Nano）：先確保模擬中兩項 SOTA 的推理延遲 <30ms。
2. 針對 v5_RL_best 進行「存活條件 IAE 分析」（只統計 survive≥250 步的 episodes），驗證 1.224m 是否為真精度。
3. 課程參數重新設計：更保守的位置邊界增長斜率 + 保留 lambda_bc > 0 防止過早遠離 BC prior。
